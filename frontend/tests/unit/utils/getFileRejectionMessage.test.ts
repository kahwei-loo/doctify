...
describe("getFileRejectionMessage", () => {
  it("returns unsupported type message for file-invalid-type", () => {
    ...
    expect(getFileRejectionMessage(rejection)).toBe('"report.exe" has an unsupported file type');
  });
  ...
  it("returns size limit message for file-too-large", () => {
    ...
    expect(getFileRejectionMessage(rejection)).toBe('"huge.pdf" exceeds the 10MB size limit');
  });
  ...
  it("returns max files message for too-many-files", () => {
    ...
    expect(getFileRejectionMessage(rejection)).toBe("Too many files selected (max 20)");
  });
  ...
  it("uses error.message for unknown error codes", () => {
    ...
    expect(getFileRejectionMessage(rejection)).toBe('"file.pdf": Something went wrong');
  });
  ...
  it("returns single message without comma separator", () => {
    const rejection = createRejection("ok.pdf", [{ code: "file-too-large", message: "Too large" }]);
  ...
  it("handles empty file name gracefully", () => {
    ...
    expect(getFileRejectionMessage(rejection)).toBe('"" has an unsupported file type');
  });
});
