...
import { FileStack, Globe, FileText, MessageSquare, Database, type LucideIcon } from "lucide-react";
...
const sourceTypeConfigs: Record<DataSourceType, SourceTypeConfig> = {
  uploaded_docs: {
    ...
    getMetadata: (source) => {
      ...
      return embCount > 0
        ? `${docCount} doc${docCount !== 1 ? "s" : ""} \u00B7 ${embCount} chunks`
      ...
      if (docs.length > 0) {
        return (
          docs
            .slice(0, 3)
            .map((d) => d.filename)
            .join(", ") + (docs.length > 3 ? ` +${docs.length - 3} more` : "")
        );
      }
      return "";
    },
