...
const ChunkItem: React.FC<{ embedding: Embedding; index: number }> = ({ embedding, index }) => {
...
function getEmptyStateHint(source?: DataSource): { message: string; detail?: string } {
  ...
  switch (source.type) {
    case "uploaded_docs": {
      ...
      const hasOnlyUnsupported =
        docs.length > 0 &&
        docs.every((d) => d.type.startsWith("image/") || d.type === "application/octet-stream");
      if (hasOnlyUnsupported) {
        return {
          message: "Image files cannot be embedded yet",
          detail:
            "Only text-based files (.txt, .md, .csv, .json) and digital PDFs with a text layer are supported for embedding. Image and scanned-PDF support (OCR) is planned for a future release.",
        };
      }
      return {
        message: "No chunks yet — content hasn't been embedded",
        detail:
          'Click "Re-embed" to generate embeddings. Note: image files and scanned PDFs without a text layer will be skipped.',
      };
    }
    ...
    case "structured_data":
      return {
        ...
        detail:
          "This data source is designed for NL-to-SQL analytics. Use the Chat panel to ask questions about this dataset — it queries the data directly rather than through vector search.",
      };
    ...
    case "website": {
      ...
      if (pagesCrawled === 0) {
        return {
          ...
          detail:
            'The website hasn\'t been crawled. Click "Re-embed" to start crawling and embedding the website content.',
        };
      }
      return { message: "No chunks yet — content hasn't been embedded" };
    }
...
export const ChunksSection: React.FC<ChunksSectionProps> = ({ embeddings, source }) => {
  if (embeddings.length === 0) {
    const hint = getEmptyStateHint(source);
    return (
      <div className="rounded-lg border border-dashed bg-muted/30 p-4 text-center">
        <Layers className="h-6 w-6 text-muted-foreground/50 mx-auto mb-1.5" />
        <p className="text-xs text-muted-foreground">{hint.message}</p>
        {hint.detail && (
          <div className="mt-2 flex items-start gap-1.5 text-left mx-auto max-w-sm">
            ...
            <p className="text-[11px] text-muted-foreground/60 leading-relaxed">{hint.detail}</p>
          </div>
        )}
      </div>
    );
