...
export const mockKnowledgeBaseApi = {
  ...
  async triggerCrawl(_dsId: string): Promise<{ data: { task_id: string } }> {
    await delay(300);
  ...
  async listEmbeddings(_kbId: string, params: PaginationParams): Promise<EmbeddingListResponse> {
    await delay(400);
  ...
  async generateEmbeddings(_dsId: string): Promise<GenerateEmbeddingsResponse> {
    await delay(500);
    return {
      success: true,
      data: {
  ...
  async testQuery(_kbId: string, request: TestQueryRequest): Promise<TestQueryResponse> {
    await delay(800);
  ...
  async getDatasetPreview(
    ...
    const allRows = [
      ...
      [3, "Product C", "Electronics", 149.5, "2024-03-10"],
      [4, "Product D", "Home", 89.0, "2024-04-05"],
      [5, "Product E", "Clothing", 34.99, "2024-05-12"],
      [6, "Product F", "Home", 199.0, "2024-06-18"],
...
const USE_MOCK =
  import.meta.env.VITE_USE_MOCK_KB === "true" || localStorage.getItem("demo_mode") === "true";
