...
const formatFieldValue = (value: any, fieldKey: string, currency?: string): string => {
  ...
  if ((lowerKey.includes('total') || lowerKey.includes('amount') || lowerKey.includes('price')) && typeof value === 'number') {
...
export const ExtractedDataView: React.FC<ExtractedDataViewProps> = ({
  ...
  const businessFields = useMemo(() => {
    ...
    return Object.entries(result.extracted_data)
      .filter(([key]) => {
        ...
        if (key.toLowerCase().includes('lineitems') || key.toLowerCase().includes('line_items')) return false;
        return true;
      })
      .map(([key, value]) => ({
  ...
  const lineItems = useMemo(() => {
    ...
    const lineItemsKey = Object.keys(result.extracted_data).find(key =>
      key.toLowerCase().includes('lineitems') || key.toLowerCase().includes('line_items')
    );
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b bg-card/50 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className={cn(
              ...
              isLowConfidence ? "bg-yellow-500/10 text-yellow-600" : "bg-green-500/10 text-green-600"
            )}>
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            ...
            {technicalData?.tokens && (
              <span className="flex items-center gap-1.5">
                ...
                {formatTokens(technicalData.tokens.total_tokens || technicalData.tokens.totalTokens || 0)} ({estimateCost(technicalData.tokens.total_tokens || technicalData.tokens.totalTokens || 0)})
              </span>
            )}
            <Button
              variant="ghost"
            ...
            >
              {showTechnicalDetails ? <ChevronUp className="h-3 w-3 mr-1" /> : <ChevronDown className="h-3 w-3 mr-1" />}
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1.5 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{formatConfidence(technicalData.confidence)}</span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="space-y-1.5 text-sm">
                    {technicalData.processTime && (
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatDuration(technicalData.processTime)}</span>
                      </div>
                    )}
                    {technicalData.tokens && (
                      <>
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">
                            {formatTokens(technicalData.tokens.prompt_tokens || technicalData.tokens.promptTokens || 0)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Completion Tokens:</span>
                          <span className="font-medium">
                            {formatTokens(technicalData.tokens.completion_tokens || technicalData.tokens.completionTokens || 0)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Total Tokens:</span>
                          <span className="font-medium">
                            {formatTokens(technicalData.tokens.total_tokens || technicalData.tokens.totalTokens || 0)}
                          </span>
                        </div>
                      </>
                    )}
