...
/**
 * Parse extraction config from project (legacy - for backward compatibility)
 */
const parseExtractionConfig = (project: Project | null): ExtractionConfig => {
  if (!project?.extraction_config) {
    return { ...DEFAULT_EXTRACTION_CONFIG };
  }

  const config = project.extraction_config as Partial<ExtractionConfig>;

  return {
    extraction_fields: Array.isArray(config.extraction_fields)
      ? config.extraction_fields
      : [],
    ocr_enabled: config.ocr_enabled ?? true,
    ai_model: config.ai_model ?? 'openai/gpt-4o-mini',
...
const validateConfig = (config: ProjectConfig): string[] => {
  ...
  config.fields.forEach((field, index) => {
    ...
    if (field.name && !/^[a-zA-Z][a-zA-Z0-9_]*$/.test(field.name)) {
      errors.push(`Field "${field.name}" has invalid name format (must start with letter, only letters, numbers, underscores)`);
    }
...
export const ProjectConfigModal: React.FC<ProjectConfigModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'fields' | 'tables' | 'output' | 'prompt' | 'processing'>('fields');
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        >
          ...
          <TabsContent
            value="fields"
            className="flex-1 overflow-auto px-6 py-4 m-0"
          >
            <div className="space-y-6">
              <InfoTip>
                Define the fields you want to extract from documents. The AI will
                use these field definitions to structure the extracted data.
              </InfoTip>
                  ...
                  <AccordionContent className="pt-2 pb-4">
                    <div className="grid gap-3 sm:grid-cols-2">
                      <TemplateCard
                        ...
                        onClick={() => {
                          handleFieldsChange([
                            { id: '1', name: 'vendor_name', type: 'text', required: true, description: 'Vendor name' },
                            { id: '2', name: 'invoice_number', type: 'text', required: true, description: 'Invoice reference' },
                            { id: '3', name: 'invoice_date', type: 'date', required: true, description: 'Invoice date' },
                            { id: '4', name: 'due_date', type: 'date', required: false, description: 'Due date' },
                            { id: '5', name: 'subtotal', type: 'number', required: false, description: 'Subtotal' },
                            { id: '6', name: 'tax_amount', type: 'number', required: false, description: 'Tax amount' },
                            { id: '7', name: 'total_amount', type: 'number', required: true, description: 'Total' },
                          ]);
                        }}
                      />
                      <TemplateCard
                        ...
                        onClick={() => {
                          handleFieldsChange([
                            { id: '1', name: 'store_name', type: 'text', required: true, description: 'Store name' },
                            { id: '2', name: 'receipt_date', type: 'date', required: true, description: 'Date' },
                            { id: '3', name: 'subtotal', type: 'number', required: false, description: 'Subtotal' },
                            { id: '4', name: 'tax', type: 'number', required: false, description: 'Tax' },
                            { id: '5', name: 'total', type: 'number', required: true, description: 'Total' },
                          ]);
                        }}
                      />
                    </div>
          ...
          >
            <div className="space-y-6">
              <InfoTip>
                ...
                Each table has columns that define the data structure.
              </InfoTip>
          ...
          >
            <div className="space-y-6">
              <InfoTip>
                Configure how documents are processed. These settings affect OCR
                quality, AI model selection, and output format.
              </InfoTip>
              ...
              <ProcessingSettingsComponent
                ...
                onOcrEnabledChange={(enabled) => handleProcessingChange({ ...processingSettings, ocr_enabled: enabled })}
                onAiModelChange={(model) => handleProcessingChange({ ...processingSettings, ai_model: model })}
                onLanguageChange={(lang) => handleProcessingChange({ ...processingSettings, language: lang })}
                onOutputFormatChange={(format) => handleProcessingChange({ ...processingSettings, output_format: format })}
              />
            </div>
          </TabsContent>
        </Tabs>
