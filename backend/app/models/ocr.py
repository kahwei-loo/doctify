...
class ExtractionConfig(BaseModel):
    ...
    detect_tables: Optional[bool] = Field(
        True,
        description="Whether to detect and extract tables"
    )
    detect_forms: Optional[bool] = Field(
        True,
        description="Whether to detect and extract form fields"
    )
    detect_handwriting: Optional[bool] = Field(
        False,
        description="Whether to attempt handwriting recognition"
    )
    detect_barcodes: Optional[bool] = Field(
        False,
        description="Whether to detect barcodes and QR codes"
    )
    ...
    include_coordinates: Optional[bool] = Field(
        False,
        description="Whether to include text coordinates in output"
    )
    include_confidence_scores: Optional[bool] = Field(
        False,
        description="Whether to include confidence scores for each text block"
    )
    ...
    def validate_language(cls, v):
        ...
        if v:
            ...
            allowed_languages = [
                ...
                'pt', 'it', 'nl', 'pl', 'tr', 'vi', 'th', 'id', 'ms', 'tl',
                'en-US', 'en-GB', 'zh-CN', 'zh-TW', 'es-ES', 'es-MX', 'fr-FR',
                'de-DE', 'ja-JP', 'ko-KR', 'pt-BR', 'pt-PT'
            ]
            if v not in allowed_languages:
                raise ValueError(f"Language '{v}' not supported. Must be one of: {', '.join(allowed_languages[:10])}...")
        return v
    ...
    def validate_page_range(cls, v):
        ...
        if v:
            ...
            if not all(c.isdigit() or c in ['-', ','] for c in v):
                raise ValueError("Page range must only contain digits, hyphens, and commas")
...
class ProcessDocumentRequest(BaseModel):
    ...
    extraction_config: Optional[ExtractionConfig] = Field(
        None,
        description="OCR extraction configuration (validated for security)"
    )
...
class OCRResult(BaseModel):
    ...
    forms: Optional[List[Dict[str, Any]]] = Field(None, description="Extracted form fields")
