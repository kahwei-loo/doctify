...
class WidgetConfig(BaseModel):
    ...
    primary_color: str = Field(default="#3b82f6", description="Primary widget color (hex)")
...
class AssistantCreate(BaseModel):
    ...
    ai_model_config: Optional[ModelConfig] = Field(None, description="AI model configuration", alias="model_config")
    widget_config: Optional[WidgetConfig] = Field(None, description="Widget appearance config")
    knowledge_base_id: Optional[str] = Field(None, description="Connected knowledge base ID")
...
class AssistantResponse(BaseModel):
    ...
    ai_model_config: Dict[str, Any] = Field(..., description="AI model configuration", alias="model_configuration")
...
class AssistantWithStats(BaseModel):
    ...
    unresolved_count: int = Field(default=0, description="Unresolved conversation count")
    created_at: datetime = Field(..., description="Creation timestamp")
    updated_at: datetime = Field(..., description="Last update timestamp")
...
class AssistantStatsResponse(BaseModel):
    ...
    total_conversations: int = Field(..., description="Total conversations across all assistants")
    unresolved_conversations: int = Field(..., description="Total unresolved conversations")
...
class AssistantAnalyticsResponse(BaseModel):
    ...
    resolution_rate: float = Field(default=0.0, description="Resolution rate percentage")
...
class PublicChatRequest(BaseModel):
    ...
    session_id: str = Field(..., min_length=1, max_length=128, description="Session ID from localStorage")
    content: str = Field(..., min_length=1, max_length=4000, description="Message content")
    context: Optional[Dict[str, Any]] = Field(None, description="Additional context (page URL, etc.)")
